...
function showMenu() {
    ...
    for (let category in menuCategories) {
        ...
        menuCategories[category].forEach(dish => {
            menuText += `🍽️ ${dish.name} - ${dish.price} VND\n   📜 ${dish.description}\n`;
        });
    }
    alert(menuText);
}
    ...
    for (let category in menuCategories) {
        menuCategories[category].forEach(dish => {
            if (dish.name.toLowerCase().includes(name.toLowerCase())) {
                results.push(`📌 ${category} - 🍽️ ${dish.name} - ${dish.price} VND\n   📜 ${dish.description}`);
            }
        });
    }
...
while (true) {
    ...
    switch (choice) {
        ...
        case 6: alert("Hẹn gặp lại!"); break;
        default: alert("Lựa chọn không hợp lệ, vui lòng nhập lại!");
    }
    if (choice === 6) break;
}
